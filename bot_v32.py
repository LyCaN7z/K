...
NUM_QUEUE_WORKERS     = int(os.getenv("NUM_QUEUE_WORKERS", "3"))   # parallel queue workers
...
def _upload_gofile(zip_path: str, progress_cb=None) -> str | None:
    ...
    try:
        ...
        logger.debug("gofile upload error: %s", e)
        return None
...
async def _send_large_file(
    ...
    for _uploader, _host_name in [
        ...
        try:
            ...
            logger.debug("filehost %s failed: %s", _host_name, e)

    # ── Fallback: split into parts ────────────────────────────────────
    logger.warning("All file hosts failed for %s — falling back to split", fname)
    if progress_cb:
        progress_cb(f"⚠️ File hosts failed — split လုပ်နေပါတယ်...")
...
async def cmd_apitest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _active_scans[uid] = "API Token Test"
    domain = urlparse(url).netloc
    msg    = await update.effective_message.reply_text(
